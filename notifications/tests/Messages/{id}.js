...
Test('/Messages/{id}', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         */
        t.test('test getMessage get operation', function (t) {
            Mockgen().requests({
                path: '/Messages/{id}',
                operation: 'get'
            }, function (err, mock) {
         ...
         * responses: 200, 400, 401, 403, 404, 429
         */
        t.test('test updateMessage patch operation', function (t) {
            Mockgen().requests({
                path: '/Messages/{id}',
                operation: 'patch'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/Messages/{id}']['patch']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
         ...
         */
        t.test('test deleteMessage delete operation', function (t) {
            Mockgen().requests({
                path: '/Messages/{id}',
                operation: 'delete'
            }, function (err, mock) {
