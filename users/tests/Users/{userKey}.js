...
/**
 * Test for /Users/{userKey}
 */
Test('/Users/{userKey}', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         */
        t.test('test getUser get operation', function (t) {
            Mockgen().requests({
                path: '/Users/{userKey}',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/Users/{userKey}']['get']['responses']['200']['schema']);
        ...
        t.test('test updateUser patch operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/Users/{userKey}']['patch']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
         ...
         * responses: 200, 400, 401, 403, 404, 409, 502, 504
         */
        t.test('test replaceUser put operation', function (t) {
            Mockgen().requests({
                path: '/Users/{userKey}',
                operation: 'put'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/Users/{userKey}']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
